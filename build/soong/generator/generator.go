...
func init() {
	android.RegisterModuleType("skydragon_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = skydragonExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = skydragonExpandVariables(ctx, dep_file)
	...
	cmd := skydragonExpandVariables(ctx, String(g.properties.Cmd))
