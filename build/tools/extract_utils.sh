...
function setup_vendor() {
    ...
    export SKYDRAGON_ROOT="$3"
    if [ ! -d "$SKYDRAGON_ROOT" ]; then
        echo "\$SKYDRAGON_ROOT must be set and valid before including this script!"
        exit 1
    fi

    export OUTDIR=vendor/"$VENDOR"/"$DEVICE"
    if [ ! -d "$SKYDRAGON_ROOT/$OUTDIR" ]; then
    ...
    export PRODUCTMK="$SKYDRAGON_ROOT"/"$OUTDIR"/"$VNDNAME"-vendor.mk
    export ANDROIDBP="$SKYDRAGON_ROOT"/"$OUTDIR"/Android.bp
    export ANDROIDMK="$SKYDRAGON_ROOT"/"$OUTDIR"/Android.mk
    export BOARDMK="$SKYDRAGON_ROOT"/"$OUTDIR"/BoardConfigVendor.mk
...
function oat2dex() {
    ...
    if [ -z "$BAKSMALIJAR" ] || [ -z "$SMALIJAR" ]; then
        export BAKSMALIJAR="$SKYDRAGON_ROOT"/prebuilts/tools-slim/common/smali/baksmali.jar
        export SMALIJAR="$SKYDRAGON_ROOT"/prebuilts/tools-slim/common/smali/smali.jar
    fi

    if [ -z "$VDEXEXTRACTOR" ]; then
        export VDEXEXTRACTOR="$SKYDRAGON_ROOT"/prebuilts/tools-slim/"${HOST,,}"-x86/bin/vdexExtractor
    fi

    if [ -z "$CDEXCONVERTER" ]; then
        export CDEXCONVERTER="$SKYDRAGON_ROOT"/prebuilts/tools-slim/"${HOST,,}"-x86/bin/compact_dex_converter
    fi
    ...
    if [ ! -f "$SKYDRAGON_TARGET" ]; then
        return;
    fi

    if grep "classes.dex" "$SKYDRAGON_TARGET" >/dev/null; then
    ...
    for ARCH in $ARCHES; do
        ...
        elif [[ "$SKYDRAGON_TARGET" =~ .jar$ ]]; then
...
function extract() {
    ...
    local OUTPUT_ROOT="$SKYDRAGON_ROOT"/"$OUTDIR"/proprietary
    ...
    if [ -f "$SRC" ] && [ "${SRC##*.}" == "zip" ]; then
        ...
        if [ "$MD5" != "$OLDMD5" ]; then
            ...
            # If OTA is block based, extract it.
                ...
                python "$SKYDRAGON_ROOT"/vendor/skydragon/build/tools/sdat2img.py "$DUMPDIR"/system.transfer.list "$DUMPDIR"/system.new.dat "$DUMPDIR"/system.img 2>&1
...
function extract_firmware() {
    ...
    local COUNT=${#FILELIST[@]}
    local SRC="$2"
    local OUTPUT_DIR="$SKYDRAGON_ROOT"/"$OUTDIR"/radio
