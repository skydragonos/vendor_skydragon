...
function oat2dex() {
    ...
    if [ -z "$OATDUMP" ] || [ -z "$VDEXEXTRACTOR" ]; then
        ...
        export VDEXEXTRACTOR="$CM_ROOT"/vendor/skydragon/build/tools/"$HOST"/vdexExtractor
   fi

    if [ -z "$CDEXCONVERTER" ]; then
        export CDEXCONVERTER="$CM_ROOT"/vendor/skydragon/build/tools/"$HOST"/compact_dex_converter
    fi
    ...
    if [ -f "$SRC" ] && [ "${SRC##*.}" == "zip" ]; then
        ...
        if [ "$MD5" != "$OLDMD5" ]; then
            ...
            # If OTA is block based, extract it.
                ...
                python "$CM_ROOT"/vendor/skydragon/build/tools/sdat2img.py "$DUMPDIR"/system.transfer.list "$DUMPDIR"/system.new.dat "$DUMPDIR"/system.img 2>&1
